...
public class QQWebsocketListener {
    ...
    @Autowired
    private WbProperties wb;

    @Bean
    public WebSocketClient webSocketClient() {
        uri = "ws://" + wb.getUrl() + ":" + wb.getPort() + "/" + wb.getAdapter();
        if (wb.getEnableVerify().equals("true")) {
            uri = uri + "verifyKey=" + wb.getVerifyKey() + "&";
        }
        uri = uri + "qq=" + wb.getQq();
                ...
                public void onMessage(String messageget) {
                    ...
                    try {
                        ...
                        if (qqMassageData.data.type.equals("GroupMessage")) {
                            ...
                            System.out.println("[未定义解析类型]" + message);
                        }
